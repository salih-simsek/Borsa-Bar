...
const AdminPage = () => {
        ...
        const unsubSession = onSnapshot(sessionRef, snap => {
          if (snap.exists()) {
            ...
            if (data.session_token && data.session_token !== currentSessionId) {
              alert(
                'Güvenlik Uyarısı: Hesabınıza başka bir cihazdan giriş yapıldı. Oturumunuz kapatılıyor.'
              );
              auth.signOut();
              navigate('/login');
            }
  ...
  // --- OTO PİYASA ---
  useEffect(() => {
    // Crash aktifken oto piyasa tamamen devre dışı, lucky varken sadece lucky ürüne dokunma
    if (!simActive || products.length === 0 || systemState === 'CRASH' || !user || licenseStatus !== 'active') {
      return;
    }
    ...
    const intervalId = setInterval(async () => {
      ...
      products.forEach(p => {
        // Şanslı ürün süresince fiyatı sabit kalsın
      ...
      if (hasUpdates) {
        try {
          ...
          console.error('Oto Piyasa Hatası:', err);
        }
      }
    }, ONE_MINUTE);
  ...
  const handleLuckyStart = async () => {
    ...
    // Snapshot sadece bilgi amaçlı (geri alma değil)
    const baseRaw = winner.rawPrice ?? winner.price ?? winner.min ?? 0;
    const basePrice = winner.price ?? baseRaw;
    const snapshot = [{ id: winner.id, rawPrice: baseRaw, price: basePrice }];
  ...
  const handleLuckyEnd = async (fromTimer = false) => {
    ...
    // Şanslı ürünü bul: önce ref, yoksa isLucky true olan
    const luckyIdFromRef = luckyProductIdRef.current;
    const luckyFromState = products.find(p => p.isLucky === true);
    const luckyId = luckyIdFromRef || (luckyFromState ? luckyFromState.id : null);
    ...
    try {
      ...
      if (!fromTimer) {
        alert('Şanslı ürün modu sonlandırıldı. Şanslı ürün başlangıç fiyatına döndü.');
      }
    }
  };
  ...
  const handleCrashEnd = async (fromTimer = false) => {
    ...
    try {
      ...
      // Crash bitince TÜM ürünlerin fiyatı başlangıç değerine dönsün
      products.forEach(p => {
      ...
      if (!fromTimer) {
        alert('Crash modu sonlandırıldı. Tüm fiyatlar başlangıç değerlerine döndü.');
      }
    }
  };
  ...
  const processPayment = async method => {
    ...
    cart.forEach(item => {
      ...
      if (!isImmune) {
        ...
        // Oto piyasa açıksa fiyat artışı, değilse sabit
        if (simActive) {
          if (currentP.type === 'HIGH') {
            ...
            // LOW ürünler deterministik artıyor
            rawAfter = rawBase + item.qty;
          }
        }
        ...
        // Crash veya şanslı ürün döneminde fiyat sabit
  ...
  const handleProductSubmit = async e => {
    ...
    try {
      ...
      if (formData.id) {
        await updateDoc(doc(productsRef, formData.id), data);
      } else {
  ...
  const updateSystemLogo = async base64 => {
    if (!user) return;
    await setDoc(
      doc(db, 'companies', user.uid, 'system_data', 'settings'),
      { logo: base64 },
      { merge: true }
    );
  ...
  const handleChangePassword = async e => {
    ...
    try {
      ...
      console.error(err);
      alert('Hata: Çıkış yapıp tekrar deneyin.');
    }
  };
  ...
  const endOfDay = async () => {
    ...
    if (todaySnap.exists()) {
      await addDoc(collection(db, 'companies', user.uid, 'reports_archive'), {
        date: new Date().toISOString(),
        ...todaySnap.data()
      });
    }
  ...
  const fetchHistory = async () => {
    ...
    const q = query(
      collection(db, 'companies', user.uid, 'reports_archive'),
      orderBy('date', 'desc'),
      limit(20)
    );
    const s = await getDocs(q);
  ...
  const deleteProduct = async id => {
    if (!user) return;
    if (!window.confirm('Silinsin mi?')) return;
    await deleteDoc(doc(db, 'companies', user.uid, 'products', id));
  };
  ...
  const resetPrices = async () => {
    ...
    products.forEach(p => {
      if (p.startPrice != null) {
        batch.update(doc(db, 'companies', user.uid, 'products', p.id), {
          price: p.startPrice,
          rawPrice: p.startPrice,
          isLucky: false
        });
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-red-900 text-white text-center p-8">
        ...
        <p className="text-xl max-w-lg mb-8">
          Bu işletmenin lisans süresi dolmuş veya ödeme beklenmektedir.
        </p>
        <button
          onClick={() => {
            auth.signOut();
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0f1115] text-white font-sans">
      ...
      <aside className="w-64 bg-[#14161b] border-r border-gray-800 flex flex-col shrink-0 pt-6">
        <div className="p-6">
          <img src="/deepeak_ana_logo.png" className="h-16 object-contain mb-2" />
          <p className="text-[10px] text-gray-500 font-mono ml-1">SaaS Panel v10.1</p>
        </div>
        ...
        <div className="p-4 border-t border-gray-800 space-y-3">
          <button
            onClick={copyTvLink}
            className="w-full bg-blue-600/20 text-blue-400 border border-blue-600 p-2 rounded-lg text-sm font-bold flex gap-2 justify-center hover:bg-blue-600/40"
          >
          ...
          <button
            ...
            className="w-full bg-gray-800 p-2 rounded-lg text-sm font-bold flex gap-2 justify-center hover:bg-gray-700"
          >
          ...
          <div
            className={`flex items-center justify-between bg-gray-800 p-3 rounded-lg border border-gray-700 ${
              systemState === 'CRASH' ? 'opacity-30 pointer-events-none' : ''
            }`}
          >
        ...
        <div className="p-4 bg-[#0f1115] border-t border-gray-800">
          <button
            ...
            className="flex justify-center w-full p-2 text-gray-400 hover:text-white"
          >
          ...
          <div className="h-full flex w-full">
            <div className="flex-1 p-6 overflow-y-auto">
              <div className="flex items-center gap-3 mb-6">
                ...
                {/* Satış başlığının yanında Crash / Şanslı sayaç rozeti */}
                {marketMode && (
                ...
                {products.map(p => {
                  ...
                  return (
                    <div
                      key={p.id}
                      onClick={() => addToCart(p)}
                      className={`bg-[#1a1d24] p-4 rounded-xl border cursor-pointer hover:border-[#FF3D00] shadow-lg ${
                        p.stock <= 0
                          ? 'opacity-50 grayscale pointer-events-none border-[#FF3D00]'
                          : 'border-gray-800'
                      } ${p.isLucky ? 'ring-2 ring-red-500' : ''}`}
                    >
                      <div className="h-24 bg-gray-800 rounded-lg mb-3 overflow-hidden relative">
                        {isDip && (
                          <div className="absolute top-0 right-0 bg-red-600 text-white text-[10px] px-2 font-bold">
                            DİP FİYAT
                          </div>
                        )}
                        {p.image ? (
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div
                          className={`font-mono font-bold text-2xl ${
                            isDip ? 'text-red-500' : 'text-white'
                          }`}
                        >
                          {p.price}₺
                        </div>
                        <div className="text-xs text-gray-400">
                          {p.stock <= 0 ? 'TÜKENDİ' : `Stok: ${p.stock}`}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="flex-1 overflow-y-auto space-y-2 relative">
                ...
                {cart.length === 0 ? (
                  ...
                  cart.map((item, i) => (
                    <div
                      key={i}
                      className="flex justify_between items-center bg-[#14161b] p-3 rounded border border-gray-800"
                    >
                      <div>
              ...
              <div className="pt-4 border-t border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="bg-[#10b981] hover:bg-green-600 text-white py-3 rounded-lg font-bold flex justify-center gap-2"
                  >
                    <Banknote className="w-5 h-5" /> NAKİT
                  </button>
                  <button
                    onClick={() => processPayment('credit')}
                    className="bg-[#3b82f6] hover:bg-blue-600 text-white py-3 rounded-lg font-bold flex justify-center gap-2"
                  >
          ...
          <div className="h-full p-8 overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              <button
                ...
                className="bg-[#FF3D00] hover:bg-red-600 px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2"
              >
                  ...
                  <tr>
                    ...
                    <th className="p-4">
                      <div className="flex justify-between">
                        ...
                        <button
                          onClick={resetPrices}
                          className="text-[10px] bg-gray-700 px-2 rounded"
                        >
                          Fiyatları Sıfırla
                        </button>
                      </div>
                      ...
                      >
                        ...
                        <td className="p-4">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                            ...
                            <div className="text-gray-400 text-xs">
                              Güncel:{' '}
                              <span className={isDip ? 'text-red-500 font-bold' : ''}>
                                {p.price}₺
                              </span>
                            </div>
                            <div className="text-gray-400 text-xs">
          ...
          <div className="h-full p-8 overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button
                  onClick={openLogs}
                  className="bg-blue-900/50 hover:bg-blue-800 text-blue-200 border border-blue-700 px-4 py-2 rounded-lg text-sm font-bold flex gap-2"
                >
                  <FileText className="w-4 h-4" /> Sistem Kayıtları
                </button>
                <button
                  onClick={fetchHistory}
                  className="bg-gray-700 px-4 py-2 rounded-lg text-sm font-bold flex gap-2"
                >
                  <History className="w-4 h-4" /> Arşiv
                </button>
                <button
                  onClick={endOfDay}
                  className="bg-red-900 text-red-100 border border-red-700 px-4 py-2 rounded-lg text-sm font-bold flex gap-2"
                >
                  <Archive className="w-4 h-4" /> Günü Bitir
                </button>
              </div>
            </div>
                ...
                <tbody>
                  {salesHistory.map(sale => (
                    <tr key={sale.id} className="border-b border-gray-800">
                      ...
                      <td className="p-3 text-right font-bold text-[#FFB300]">
                        {sale.total}₺
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-[#1a1d24] p-6 rounded-xl w-full max-w-md border border-gray-700">
            ...
            <form onSubmit={handleProductSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <div className="text-xs text-gray-300 font-semibold">Ürün İsmi</div>
                <input
                  placeholder="Örn: Kola 33cl"
                  value={formData.name}
              ...
              <div className="space-y-2">
                <div className="text-xs text-gray-300 font-semibold">Ürün Görseli</div>
                <div className="flex gap-2">
                  ...
                  {formData.image && (
                    <img src={formData.image} className="h-10 w-10 rounded" />
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  <div className="text-xs text-gray-300 font-semibold">Başlangıç Fiyatı</div>
                  <input
                    type="number"
                    placeholder="Örn: 120"
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-300 font-semibold">Minimum Fiyat</div>
                  <input
                    type="number"
                    placeholder="Örn: 80"
                    value={formData.min}
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-300 font-semibold">Maksimum Fiyat</div>
                  <input
                    type="number"
                    placeholder="Örn: 180"
                    value={formData.max}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <div className="text-xs text-gray-300 font-semibold">Stok Adedi</div>
                  <input
                    type="number"
                    placeholder="Örn: 50"
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-300 font-semibold">Kar Oranı</div>
                  <select
                    value={formData.type}
      ...
      {isLogsOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#1a1d24] p-6 rounded-xl w-full max-w-lg h-[70vh] flex flex-col border border-gray-700">
            <div className="flex justify-between mb-4">
              <h3 className="text-xl font-bold">Sistem Kayıtları</h3>
              <button onClick={() => setIsLogsOpen(false)}>
                <X />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto font-mono text-xs">
              {systemLogs.map(log => (
                <div
                  key={log.id}
                  className="border-b border-gray-800 p-2 text-gray-400"
                >
      ...
      {isHistoryOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#1a1d24] p-6 rounded-xl w-full max-w-lg h-[70vh] flex flex-col border border-gray-700">
            <div className="flex justify-between mb-4">
              <h3 className="text-xl font-bold">Arşiv</h3>
